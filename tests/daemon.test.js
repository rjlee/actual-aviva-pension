...
describe('runDaemon', () => {
  ...
  it('starts web UI when ui flag is true', () => {
    runDaemon({ verbose: false, ui: true, httpPort: 3001, debug: false });
    expect(startWebUi).toHaveBeenCalledWith(3001, false, false);
  });

  it('starts web UI when HTTP_PORT env var is set', () => {
    process.env.HTTP_PORT = '4000';
    runDaemon({ verbose: true, ui: false, httpPort: 4000, debug: false });
    expect(startWebUi).toHaveBeenCalledWith(4000, true, false);
  });

  it('always schedules sync jobs', () => {
    const spy = jest.spyOn(require('../src/daemon'), 'scheduleSync').mockImplementation(() => {});
    runDaemon({ verbose: true, ui: false, httpPort: 0, debug: false });
